<template>
    <div>
      ...
      <div v-if="applications.length === 0" class="no-applications-message" style="margin: 40px 20px;">You have no pending applications.</div>
          ...
          <div v-for="application in applications" :key="application.Application_ID" class="application-item">
            <div class="application-details">
              <div class="application-title">
                ...
                <span :class="['application-status', application.Application_Status.toLowerCase()]">{{ application.Application_Status }}</span>
              </div>
              ...
              <div class="application-date application-date-right">Application Date: {{ formatDate(application.Application_Date) }}</div>
            </div>
          </div>
        </div>
      </div>
  ...
  .application-title {
    font-size: 20px;
    font-weight: bold;
    margin-bottom: 15px; /* Increase vertical space between title and status */
  }
